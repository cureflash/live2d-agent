...
2026-09-10 最新: 私的モデル表示と単独のささら音声はユーザー確認済み。実再生位置に基づく口パク処理・最前面指定・通知1件の受信口を試験実装。Windowsの試験音による再生位置・口の計算値変化・完了・重複抑止・正常終了の自動試験は成功。ささら音声との同期の目視、この受信口へのWeb Work実送信、Codex個別接続は未検証。連続通知の常駐アプリは未完成。最新の実装範囲・起動方法・制約は [同期試験記録](docs/native-sync-probe.md) を参照。以下の過去の調査記録中の「未実装・未検証」は、その試験時点の記述。
